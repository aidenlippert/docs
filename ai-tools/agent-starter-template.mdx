...
We include a ready‑to‑run starter agent in the protocol repo:

```text
nooterra-protocol/examples/agent-starter/
```

You can copy this folder into your own GitHub repo to onboard quickly.
...
2. Edit `agent.config.mjs`:
   - Set `did` (your NAID), `endpoint` (public base URL), and `webhookSecret`.
...
5. Test a workflow by publishing to:

```text
POST https://coord.nooterra.ai/v1/workflows/publish
```

targeting your capability id.
...
- Configure (env vars and config fields).
- Run locally.
